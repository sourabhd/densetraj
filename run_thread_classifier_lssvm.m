function [out] = run_thread_classifier_lssvm(param)
...
%        run_thread_classifier_lssvm(param.dset_dir, param.base_dir, ...
%        param.cl, param.tr, param.te, ...
%        param.tr_v, param.te_v, param.fileorder, ...
%        param.retain_frac_threads, ...
%        param.Linear_K_video, param.Linear_KK_video) 
    ...
    which svmtrain
    which svmpredict
    param.fileorder.train_files
    param.fileorder.test_files
    size(param.tr.train_fv)
    size(param.te.test_fv)
    size(param.tr_v.train_fv)
    size(param.te_v.test_fv)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    labels_dict_file_train = ...
        ...
        param.dset_dir, '/', 'ClipSets', '/', param.cl, '_', 'train', '.txt');
    fprintf('%s\n', labels_dict_file_train); 
    [training_labels_fname_video, training_labels_vector_video] = ...
        textread(labels_dict_file_train, '%s %d');
    tr_sz = size(param.tr.train_fv);
    num_tr = tr_sz(1);
    tr_fname = {};
    training_labels_vector = zeros(num_tr,1);
    for i = 1:num_tr
        tr_fname{i} = ...
            char(training_labels_fname_video(param.tr.tr_r2t(i).fileNum,:));
        training_labels_vector(i,1) = ...
            training_labels_vector_video(param.tr.tr_r2t(i).fileNum, :);
    end
    ...
    labels_dict_file_test = ...
        sprintf('%s%s%s%s%s%s%s%s', ...
        param.dset_dir, '/', 'ClipSets', '/', param.cl, '_', 'test', '.txt');
    fprintf('%s\n', labels_dict_file_test); 
    [testing_labels_fname_video, testing_labels_vector_video] = ...
        textread(labels_dict_file_test, '%s %d');
    te_sz = size(param.te.test_fv);
    num_te = te_sz(1);
    te_fname = {};
    testing_labels_vector = zeros(num_te,1);
    for i = 1:num_te
        te_fname{i} = ...
            char(testing_labels_fname_video(param.te.te_r2t(i).fileNum,:));
        testing_labels_vector(i,1) = ...
            testing_labels_vector_video(param.te.te_r2t(i).fileNum, :);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %param.Linear_K_video = param.tr_v.train_fv * param.tr_v.train_fv';
    ...
    cv_video = ...
        ...
        [ (1:num_tr_video)' param.Linear_K_video], ...
    ...
    cv_video = do_binary_cross_validation(training_labels_vector_video, ...
        [ (1:num_tr_video)' param.Linear_K_video], opt_video, n_fold_video); 

    lambda = 10^-3 * num_tr_video; 
    [alphas,b, cvErrs, cvWs, cvBs] = ...
        ML_Ridge.kerRidgeReg_cv(param.Linear_K_video, ...
        training_labels_vector_video, lambda, ones(num_tr_video,1));
    w = param.Linear_K_video*alphas;            
    decision_values_video = param.Linear_KK_video * alphas + b; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:num_tr_video
        num_tr_vid_i = length(param.tr.tr_threads_with_fv{i});
        for j = 1:num_tr_vid_i
            t_key = sprintf('%8d_%8d', i, param.tr.tr_threads_with_fv{i}{j});
...
%  Some declarations
    ...
    feat_dim = size(param.tr.train_fv,2);
...
%%     Linear_KK = param.tr.train_fv * param.tr_v.train_fv';   % kernel for validation
...
%%         num_tr_vid_v = length(param.tr.tr_threads_with_fv{v});
%%         if training_labels_vector_video(v,:) == 1                                                     % if +ve
%%             K = param.Linear_K_video;                                                                       % drop the video fv
%%             K(v,:) = [];
%%             K(:,v) = [];
...
%%             %n = max(round(param.retain_frac_threads * num_tr_vid_v), 1); 
...
%%             avg_feature_video = mean(param.tr.train_fv(sorted_val_ix_reduced,:), 1);
...
%%                 val_ix(j,:) = thread2row_map(thread_key);
%%             end
%%             avg_feature_video = mean(param.tr.train_fv(val_ix,:), 1);
%%             nm_avg_feature_video = norm(avg_feature_video);
%%             if nm_avg_feature_video ~= 0
%%                 avg_feature_video = avg_feature_video / nm_avg_feature_video; 
%%             else
...
%%     % Linear_K_pruned  = param.tr.train_fv(tr_ix_pruned,:) * param.tr.train_fv(tr_ix_pruned,:)';
%%     % Linear_KK_pruned = param.te.test_fv * param.tr.train_fv(tr_ix_pruned,:)';
%% 
...
%% 
%%     num_test_video = size(param.te.te_name,1);
%%     thread_based_features_video = zeros(num_test_video,feat_dim);
%%     for k = 1:num_te_video
%%         tfv = zeros(1, feat_dim);
%%         for l = 1:length(param.te.te_t2r_hmap{k})
%%             tfv = tfv + param.te.test_fv(param.te.te_t2r_hmap{k}{l}(1), :);
%%         end
...
%% %        dv_k = zeros(length(param.te.te_t2r_hmap{k}),1);
%% %        for l = 1:length(param.te.te_t2r_hmap{k})
%% %            dv_k(l,:) = out.decision_values(param.te.te_t2r_hmap{k}{l}(1), :);
%% %        end
